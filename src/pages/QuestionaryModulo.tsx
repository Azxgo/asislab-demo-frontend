...
export function QuestionaryModulo({ questions,
    questionaryId,
    userId,
    startTime,
    initialModule,
    ...
    const [currentModuleIndex, setCurrentModuleIndex] = useState(initialModule)

    const submitModule = async (moduleIndex: number) => {

        const module = questions[moduleIndex]
        ...
        module.preguntas.forEach((p, questionIndex) => {
        ...
        try {
            ...
            if (moduleIndex === questions.length - 1 && startTime) {
                const endTime = Date.now()
                payload.duration = (endTime - startTime) / 1000
            }

            const res = await axios.post(
                `http://localhost:5000/questions/submitModule/${questionaryId}`,
                payload,
                {
    ...
    const isModuleComplete = currentModule.preguntas.every((_, questionIndex) => {
        const questionKey = `${currentModuleIndex}-${questionIndex}`
        return !!selectedOptions[questionKey]
    })
            ...
            {questions.length > 0 ? (
                <>
                    <div className="flex flex-col border border-gray-200 rounded-xl p-3 gap-3">
                        ...
                        {questions[currentModuleIndex].preguntas.map((question, questionIndex) => {
                            const questionKey = `${currentModuleIndex}-${questionIndex}`
                                ...
                                <div key={questionIndex} className="p-6 rounded-xl shadow-sm hover:shadow-md
                                    ...
                                    <div className="flex flex-col my-3">
                                        {question.alternativas.map((alt, i) => {
                                            const isSelected = selectedOptions[questionKey] === alt.opcion
                                            return (
                                                <div key={i}>
                                                    <SingleOption
                                                        label={alt.opcion}
