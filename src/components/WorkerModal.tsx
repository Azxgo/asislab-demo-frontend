...
export function WorkerModal({ id, isOpen, onClose, onSave, setToast }: ModalProps) {
    ...
    useEffect(() => {
        if (id) {
            setIsLoading(true);
            axios
                .get(`http://localhost:5000/workers/getById/${id}`)
                .then((res) => {
                    if (res.data.success) {
    ...
    const handleSave = async () => {
        ...
        try {
            setIsLoading(true);
            if (id) {
                await axios.put(`http://localhost:5000/workers/update/${id}`, {
                    ...worker,
                    area_id: area
                });
            } else {
                await axios.post("http://localhost:5000/workers/add", {
                    ...worker,
                    area_id: area
                });
            }
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                <div className="flex flex-col">
                                                    ...
                                                    <input
                                                        ...
                                                        value={worker.rut}
                                                        onChange={e => setWorker(prev => ({ ...prev, rut: e.target.value }))}
                                                    />
                                                </div>
