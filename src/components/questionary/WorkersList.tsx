...
export function WorkersList({ id, isOpen, onClose }: WorkersListProps) {
    ...
    const fetchWorkers = async (id: string) => {
        try {
            const res = await apiClient.get(`/questions/get_stats_by_workers/${id}`)
                                                ...
                                                {workers.map((worker, i) => (
                                                    <>
                                                        <tr key={i} className="text-left hover:bg-gray-50 dark:hover:bg-zinc-700/50 transition-colors duration-200">
                                                            ...
                                                            <td className="px-4 py-1 font-medium">
                                                                {worker.module_durations
                                                                    ? formatSeconds(
                                                                        Object.values(worker.module_durations)
                                                                            .reduce((a: number, b: any) => a + Number(b), 0)
                                                                    )
                                                                    : worker.duration != null
                                                                        ? formatSeconds(worker.duration)
                                                                        : "0"}
                                                            </td>
                                                                                        ...
                                                                                        <div className={`overflow-hidden transition-all duration-500 ease-in-out
                                                                                            ...
                                                                                            <div className="mt-2 space-y-2">
                                                                                                {preguntas.sort((a: any, b: any) => {
                                                                                                    const aId = a.question_id ?? "0-0";
                                                                                                    const bId = b.question_id ?? "0-0";

                                                                                                    const aQ = parseInt(aId.split("-")[1]);
                                                                                                    const bQ = parseInt(bId.split("-")[1]);

                                                                                                    return aQ - bQ;
                                                                                                })
                                                                                                    .map((r: any, idx: number) => {
                                                                                                        const [_, q] = r.question_id.split("-");
                                                                                                        return (
                                                                                                            <div
                                                                                                                key={idx}
