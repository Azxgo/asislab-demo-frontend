...
export function AddWorkers({ id, area, isOpen, onClose }: WorkersListProps) {
    ...
    const fetchWorkersId = async (id: string) => {
        try {
            const res = await axios.get(`http://localhost:5000/workers/getAllById/${id}`);
    ...
    const handleAddWorkers = async () => {
        ...
        try {
            ...
            const res = await axios.post(
                `http://localhost:5000/workers/upload/${id}`,
                { users: usersToUpload },
    ...
    const handleRemoveWorkers = async () => {
        ...
        try {
            const csrfToken = Cookies.get("csrf_access_token")
            const res = await axios.post(`http://localhost:5000/workers/removeFromQuestionary/${id}`, {
    ...
    const handleAddOneWorker = async (workerId: string) => {
        ...
        try {
            const csrfToken = Cookies.get("csrf_access_token");
            await axios.post(
                `http://localhost:5000/workers/assign/${id}`,
                { worker_id: workerId },
                { headers: { "X-CSRF-TOKEN": csrfToken }, withCredentials: true }
            );
