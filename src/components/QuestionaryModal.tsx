    ...
    const handleResume = async () => {
        ...
        try {
            ...
            const res = await axios.post(
                `http://localhost:5000/questions/resume/${resumeId}`
            );
            ...
            if (Array.isArray(steps) && steps.length > 0) {
                ...
                for (const step of steps) {
                    setMessage(step.msg);
                    const stepRes = await postWithCsrf(`http://localhost:5000${step.url}`);
                ...
                if (area && assignWorkers) {
                    ...
                    await postWithCsrf(
                        `http://localhost:5000/workers/assign_by_area/${resumeId}`,
                        { area }
                    );
                }
                ...
                const verify = await axios.get(
                    `http://localhost:5000/questions/get_questions/${resumeId}`
                );
                ...
                if (!verify.data.questions || verify.data.questions.length === 0) {
                    ...
                    const retry = await postWithCsrf(
                        `http://localhost:5000/questions/generate_questions/${resumeId}`
                    );

                    if (retry.data.error)
                        throw new Error(retry.data.error || "Error al reintentar generación");

                    const verifyAgain = await axios.get(
                        `http://localhost:5000/questions/get_questions/${resumeId}`
                    );
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const uploadRes = await postWithCsrf(
                "http://localhost:5000/questions/upload_file",
                formData
            );
            ...
            for (const step of steps) {
                setMessage(step.msg);
                const res = await postWithCsrf(`http://localhost:5000${step.url}`);
            ...
            if (area && assignWorkers) {
                ...
                await postWithCsrf(
                    `http://localhost:5000/workers/assign_by_area/${questionary_id}`,
                    { area }
                );
            }
        ...
        const interval = setInterval(async () => {
            try {
                const res = await axios.get(
                    `http://localhost:5000/questions/status/${questionaryId}`
                );
                            ...
                            {isResuming ? (
                                ...
                                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        {/* Drag & Drop agregado */}
                                        <div className="flex flex-col items-center gap-6 p-3 h-full">
                                            ...
                                            <div className="flex justify-center items-center gap-6">
                                                ...
                                                <div className="flex gap-2 items-center">
                                                    <label className="font-medium">Num Preguntas</label>
                                                    <input
                                                        type="text"
                                                        value={numQuestions}
                                                        onChange={(e) => setNumQuestions(e.target.value)}
                                                        className="w-20 px-4 py-1.5 border border-gray-200 dark:border-zinc-400 rounded-lg"
                                                        disabled={isLoading}
                                                    />
                                                </div>
                                            </div>
