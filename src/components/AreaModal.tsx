...
export function AreaModal({ id, isOpen, onClose, onSave }: ModalProps) {
    ...
    useEffect(() => {
        if (id) {
            setIsLoading(true)
            axios.get(`http://localhost:5000/areas/get/${id}`)
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (id) {
                await axios.put(`http://localhost:5000/areas/update/${id}`, { nombre: name }, {
                    headers: { "X-CSRF-TOKEN": csrfToken },
                    withCredentials: true
                })
            } else {
                await axios.post(`http://localhost:5000/areas/create`, { nombre: name }, {
                    headers: { "X-CSRF-TOKEN": csrfToken },
                    withCredentials: true
                })
            }
